    ...
    it('should return a specific zone by ID', () => {
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          console.log(
            `✅ Zone ${zone.id}: ${zone.name} with ${zone.rooms.length} rooms`
          );
        });
    });
  });
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          if (response.body.data.roomsByZone.length > 0) {
            ...
            console.log(
              `✅ Found ${response.body.data.roomsByZone.length} rooms in zone 365`
            );
            response.body.data.roomsByZone.slice(0, 3).forEach(room => {
              console.log(
                `   - Room ${room.id}: ${room.name} (${room.exits.length} exits)`
              );
            });
          }
        });
    ...
    it('should return a specific room by ID', () => {
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          console.log(
            `✅ Room ${room.id}: ${room.name} in zone ${room.zone.name}`
          );
          console.log(
            `   Exits: ${room.exits.map(e => `${e.direction} → ${e.destination}`).join(', ')}`
          );
        });
    });
  });
        ...
        .then(response => {
          ...
          if (response.body.data.mobs.length > 0) {
            ...
            response.body.data.mobs.forEach(mob => {
              console.log(
                `   - Mob ${mob.id}: ${mob.shortDesc} (Level ${mob.level} ${mob.mobClass})`
              );
            });
          }
        });
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          if (response.body.data.objects.length > 0) {
            ...
            console.log(
              `✅ Found ${response.body.data.objects.length} objects`
            );
            response.body.data.objects.forEach(object => {
              console.log(
                `   - Object ${object.id}: ${object.shortDesc} (${object.type})`
              );
            });
          }
        });
        ...
        .then(response => {
          ...
          if (response.body.data.shops.length > 0) {
            response.body.data.shops.forEach(shop => {
              console.log(
                `   - Shop ${shop.id}: Buy ${shop.buyProfit}x, Sell ${shop.sellProfit}x`
              );
            });
          }
        });
    ...
    it('should be able to update a room', () => {
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          expect(updatedRoom.name).toBe(
            'The Northern Swamps (Updated by Test)'
          );
          expect(updatedRoom.description).toContain('automated test');
          expect(updatedRoom.updatedAt).toBeDefined();

          console.log(
            `✅ Successfully updated room ${updatedRoom.id}: ${updatedRoom.name}`
          );
        });
    });
    ...
    it('should revert the room update', () => {
      ...
      return request(app.getHttpServer())
        ...
        .then(response => {
          ...
          console.log(
            `✅ Successfully reverted room ${revertedRoom.id} to original state`
          );
        });
    });
  });
