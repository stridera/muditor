...
test.describe('Authentication System', () => {
    ...
    test.describe('User Registration', () => {
        ...
        test('should prevent duplicate email registration', async ({ page }) => {
            ...
            await expect(page.locator('.error, .text-red-500, [role="alert"]')).toContainText(/email.*already.*exists|email.*taken/i);
        });
    });
    test.describe('User Login', () => {
        ...
        test('should display login form', async ({ page }) => {
            ...
            await expect(page.locator('a:has-text("Forgot"), a:has-text("Reset")')).toBeVisible();
        });
        ...
        test('should show error for invalid credentials', async ({ page }) => {
            await page.goto('/login');
            await page.fill('input[name="username"], input[name="email"]', 'nonexistent@example.com');
            await page.fill('input[name="password"]', 'wrongpassword');
            await page.click('button[type="submit"]');
            // Should show error message
            await expect(page.locator('.error, .text-red-500, [role="alert"]')).toContainText(/invalid.*credentials|login.*failed|incorrect/i);
    ...
    test.describe('Password Reset', () => {
        test('should display forgot password form', async ({ page }) => {
            ...
            await expect(page.locator('h1')).toHaveText(/forgot.*password|reset.*password/i);
            await expect(page.locator('input[name="email"]')).toBeVisible();
            await expect(page.locator('button[type="submit"]')).toBeVisible();
        ...
        test('should send password reset email', async ({ page }) => {
            ...
            await expect(page.locator('.success, .text-green-500, [role="status"]')).toContainText(/reset.*link.*sent|check.*email/i);
        });
        ...
        test('should display reset password form with valid token', async ({ page, }) => {
            ...
            await expect(page.locator('h1')).toHaveText(/reset.*password|new.*password/i);
            await expect(page.locator('input[name="password"]')).toBeVisible();
            await expect(page.locator('input[name="confirmPassword"]')).toBeVisible();
            await expect(page.locator('button[type="submit"]')).toBeVisible();
        });
    ...
    test.describe('Role-Based Access Control', () => {
        test('should show appropriate UI elements based on user role', async ({ page, }) => {
            ...
            const adminLinks = page.locator('a:has-text("Admin"), a:has-text("Users"), a:has-text("System")');
    ...
    test.describe('Environment Management', () => {
        test('should display environment selector', async ({ page }) => {
            ...
            const envSelector = page.locator('[data-testid="environment-selector"], .environment-selector, select:has(option:has-text("Development"))');
