...
export const WorldMapZone: React.FC<WorldMapZoneProps> = ({ data }) => {
  ...
  const baseScale = Math.max(
    8,
    Math.min(25, 15 + Math.log10(roomDensity + 1) * 5)
  );
  ...
  const width =
    providedWidth ||
    Math.max(minZoneSize, Math.min(maxZoneSize, boundsWidth * baseScale + 80));
  const height =
    providedHeight ||
    Math.max(
      minZoneSize * 0.75,
      Math.min(maxZoneSize * 0.75, boundsHeight * baseScale + 80)
    );
      ...
      {isHovered && (
        ...
        >
          ...
          <div className='text-sm space-y-1'>
            <div>
              <span className='font-semibold'>Zone ID:</span> {zoneId}
            </div>
            <div>
            ...
            <div>
              <span className='font-semibold'>Bounds:</span> ({bounds.minX},{' '}
              {bounds.minY}) to ({bounds.maxX}, {bounds.maxY})
            </div>
            <div>
              <span className='font-semibold'>Center:</span> (
              {bounds.centerX.toFixed(1)}, {bounds.centerY.toFixed(1)})
            </div>
          </div>
        </div>
      )}
