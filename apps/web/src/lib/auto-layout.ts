...
function selectStartingRoom(rooms: AutoLayoutRoom[], startRoomId?: number): AutoLayoutRoom | null {
  ...
  if (selectedRoom) {
    console.log(`🎯 Smart starting room selection: room ${selectedRoom.id} (score: ${roomScores[0].score}, exits: ${selectedRoom.exits.length})`);
  }
...
function calculateLayoutQuality(positions: Record<number, LayoutPosition>, rooms: AutoLayoutRoom[]): number {
  ...
  if (overlaps.length > 0) {
    ...
    overlaps.forEach((overlap, idx) => {
      console.log(`  ${idx + 1}. Position (${overlap.position.x}, ${overlap.position.y}, ${overlap.position.z}): Rooms ${overlap.roomIds.join(', ')}`);
    });
  } else {
...
export function autoLayoutRooms(rooms: AutoLayoutRoom[], startRoomId?: number): Record<number, LayoutPosition> {
  ...
  while (queue.length > 0) {
    ...
    console.log(`🔍 Processing room ${roomId} at (${position.x}, ${position.y}) with ${room.exits.length} exits`);

    // Process each exit
    for (const exit of room.exits) {
      console.log(`🔍 Found exit: ${exit.direction} → room ${exit.toRoomId}`);
      if (exit.toRoomId === null || exit.toRoomId === undefined || visited.has(exit.toRoomId)) continue;
      ...
      if (!validateDirection(exit.direction)) {
        console.warn(`⚠️ Unknown direction: ${exit.direction}, using default offset`);
      }
      ...
      if (directionUpper === 'UP' || directionUpper === 'DOWN') {
        ...
        console.log(`🔍 Placing room ${exit.toRoomId} at (${newPosition.x}, ${newPosition.y}, Z${newPosition.z}) via ${exit.direction} exit (Z-level change: ${zChange > 0 ? '+' : ''}${zChange})`);
      } else {
        // Horizontal exits use X/Y offset mapping
        const offset = DIRECTION_OFFSETS[exit.direction];
        if (!offset) {
          console.warn(`⚠️ No offset mapping for direction: ${exit.direction}, using default east offset`);
        ...
        console.log(`🔍 Placing room ${exit.toRoomId} at (${newPosition.x}, ${newPosition.y}, Z${newPosition.z || 0}) via ${exit.direction} exit (offset: ${JSON.stringify(offset || 'calculated')})`);
      }
  ...
  for (const room of rooms) {
    if (!visited.has(room.id)) {
      ...
      if (unconnectedX > 15) { // Wrap after 6 rooms per row
        unconnectedX = 10;
  ...
  console.log(`🔍 Auto-layout complete: ${Object.keys(positions).length} total positions, ${visited.size} connected, ${unconnectedCount} unconnected`);
...
export function resolveOverlaps(positions: Record<number, LayoutPosition>): Record<number, LayoutPosition> {
  ...
  console.log(`🔍 LOCAL resolveOverlaps: Keeping original positions (overlaps allowed)`);
  return { ...positions };
}
