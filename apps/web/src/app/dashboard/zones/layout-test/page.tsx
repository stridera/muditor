...
import { autoLayoutRooms, resolveOverlaps, detectOverlaps, calculateLayoutQuality } from '@/lib/auto-layout';
...
export default function LayoutTestPage() {
  const { data: zonesData, loading: zonesLoading, error: zonesError, refetch: refetchZones } = useQuery<ZonesQueryResult>(GET_ZONES);
  const [selectedMode, setSelectedMode] = useState<'zone' | 'world'>('zone');
  const [selectedZone, setSelectedZone] = useState<number | null>(null);
  const [selectedStartRoom, setSelectedStartRoom] = useState<number | null>(null);
  ...
  const { data: roomsData, loading: roomsLoading, error: roomsError } = useQuery<RoomsQueryResult>(
    GET_ROOMS_BY_ZONE,
    {
  ...
  const runLayoutAlgorithm = () => {
    ...
    console.log(`📍 Mode: ${selectedMode}`, selectedMode === 'zone' ? `(Zone ${selectedZone})` : '(All zones)');
    console.log(`🎯 Start room: ${selectedStartRoom || 'auto-select'}`);

    // Run auto-layout
    const positions = autoLayoutRooms(currentRooms, selectedStartRoom || undefined);
    ...
    console.log(`✅ Layout complete: ${Object.keys(resolvedPositions).length} positioned, ${overlaps.length} overlaps, quality: ${quality.toFixed(2)}`);
  };
  ...
  const loading = zonesLoading || (selectedMode === 'zone' && selectedZone && roomsLoading);
  ...
  return (
    <PermissionGuard requireImmortal={true}>
      <div className="w-full -mx-4 px-6 py-6" style={{ width: 'calc(100vw - 2rem)', marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)' }}>
        {/* Header */}
        <div className="mb-6">
          <div className="flex items-center space-x-2 text-sm text-gray-500 mb-2">
            <Link href="/dashboard" className="hover:text-gray-700">Dashboard</Link>
            <span>›</span>
            <Link href="/dashboard/zones" className="hover:text-gray-700">Zones</Link>
            <span>›</span>
            <span className="text-gray-900 font-medium">Layout Test</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Room Layout Algorithm Test</h1>
          <p className="text-gray-600">Test and fine-tune the automatic room positioning algorithm</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedMode} onValueChange={(value: 'zone' | 'world') => setSelectedMode(value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSelectedStartRoom(value === 'auto' ? null : parseInt(value))}
                  >
                    <SelectTrigger>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <Badge variant={selectedMode === 'world' ? 'default' : 'secondary'}>
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">{currentRooms.length} total</div>
                </div>
                ...
                {selectedStartRoom && (
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {currentRooms.find(r => r.id === selectedStartRoom)?.name || `Room ${selectedStartRoom}`}
                    </div>
                  </div>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2 text-sm text-gray-600">
                  <p>• <strong>Quality Score:</strong> Lower is better (path length + overlap penalty)</p>
                  <p>• <strong>Smart Start:</strong> Auto-selects rooms with most connections or "entrance" keywords</p>
                  <p>• <strong>Direction Mapping:</strong> Uses 2-unit spacing to prevent overlaps</p>
                  <p>• <strong>Z-Levels:</strong> UP/DOWN exits maintain X/Y position, change Z-level</p>
                  <p>• <strong>Unconnected Rooms:</strong> Placed in separate grid area at (5,0) onwards</p>
                  <p>• <strong>Overlap Resolution:</strong> Uses spiral search pattern around conflicts</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{layoutResults.roomCount}</div>
                    ...
                    <div className="text-center">
                      <div className={`text-2xl font-bold ${layoutResults.overlaps.length > 0 ? 'text-red-600' : 'text-green-600'}`}>
            ...
            {layoutResults && layoutResults.overlaps.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg text-red-600">Overlaps Detected</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {layoutResults.overlaps.map((overlap, index) => (
                      <div key={index} className="p-3 bg-red-50 rounded-lg border border-red-200">
                        <div className="text-sm font-medium text-red-800">
                          Position ({overlap.position.x}, {overlap.position.y}, Z{overlap.position.z || 0})
                        </div>
            ...
            {layoutResults && (
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4" style={{ height: 'calc(100vh - 500px)' }}>
                {/* Room List with Positions */}
                <Card className="h-full flex flex-col overflow-hidden">
                  <CardHeader className="flex-shrink-0">
                    <CardTitle className="text-lg">Room Positions ({currentRooms.length} rooms)</CardTitle>
                  </CardHeader>
                            ...
                            >
                              <div className="flex items-center justify-between mb-2">
                                <div>
                                  ...
                                  <div className="text-sm text-gray-600">
                                    {room.name || 'Unnamed'} • {room.exits.length} exits
                                  </div>
                                </div>
                                <div className="text-right">
                                  {position ? (
                                    <div className="text-sm font-mono">
                                      ({position.x}, {position.y}, Z{position.z || 0})
                                    </div>
                                  ) : (
                                    <div className="text-sm text-red-600">Not positioned</div>
                                  )}
                                </div>
                              </div>
                              {room.exits.length > 0 && (
                                <div className="text-xs text-gray-500">
                                  ...
                                  <div className="grid grid-cols-2 gap-1">
                                    {room.exits.map((exit, exitIndex) => (
                                      <div key={exitIndex} className="flex justify-between">
                                        <span className="font-mono">{exit.direction}:</span>
                    ...
                    {currentRooms.length > 0 ? (
                      ...
                      <div className="h-96 flex items-center justify-center text-gray-500">
                        <div className="text-center">
                          ...
                          <p className="text-sm">Run the algorithm to see visualization</p>
                        </div>
                      </div>
                    )}
                  </CardContent>
