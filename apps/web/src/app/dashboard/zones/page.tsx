...
export default function ZonesPage() {
  ...
  const filteredZones = useMemo(() => {
    return zones.filter((zone) => {
      ...
      const matchesClimate = selectedClimate === 'all' || zone.climate === selectedClimate
    ...
    <div>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center text-sm text-gray-600 space-x-4">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    zone.climate === 'NONE' ? 'bg-gray-100 text-gray-700' :
                    zone.climate === 'OCEANIC' ? 'bg-blue-100 text-blue-700' :
                    zone.climate === 'TEMPERATE' ? 'bg-green-100 text-green-700' :
                    zone.climate === 'TROPICAL' ? 'bg-yellow-100 text-yellow-700' :
                    zone.climate === 'SUBARCTIC' ? 'bg-cyan-100 text-cyan-700' :
                    zone.climate === 'ARID' ? 'bg-orange-100 text-orange-700' :
                    zone.climate === 'SEMIARID' ? 'bg-amber-100 text-amber-700' :
                    zone.climate === 'ALPINE' ? 'bg-purple-100 text-purple-700' :
      ...
      {filteredZones.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">No zones found matching your criteria</p>
