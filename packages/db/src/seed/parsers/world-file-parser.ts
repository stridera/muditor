...
export class WorldFileParser {
  ...
  private async parseRoom(room: any, zoneId: number) {
        ...
        await this.prisma.roomExit.upsert({
          ...
          update: {
            ...
            destination: exitData.destination
              ? parseInt(exitData.destination.toString())
              : null,
          },
          create: {
            direction: mappedDirection,
    ...
    if (
      ...
      typeof room.extra_descriptions === 'object'
    ) {
    ...
    if (shop.selling && typeof shop.selling === 'object') {
      for (const [objectId, amount] of Object.entries(shop.selling)) {
        ...
        if (objectExists) {
          ...
          console.log(
            `⚠️  Object ${objectId} not found for shop ${createdShop.id} in zone ${zoneId}`
          );
        }
      }
    }
  ...
  private async parseMobReset(mobReset: any, zoneId: number) {
    ...
    if (!mobExists) {
      console.warn(
        `⚠️  Mob ${mobReset.id} not found for mob reset in zone ${zoneId}, skipping`
      );
      return;
    }
    ...
    if (!roomExists) {
      console.warn(
        `⚠️  Room ${mobReset.room} not found for mob reset in zone ${zoneId}, skipping`
      );
      return;
    }
    ...
    if (mobReset.carrying && Array.isArray(mobReset.carrying)) {
      for (const item of mobReset.carrying) {
        ...
        if (objectExists) {
          ...
          console.log(
            `⚠️  Object ${item.id} not found for mob carrying in zone ${zoneId}`
          );
        }
      }
    }
    ...
    if (mobReset.equipped && Array.isArray(mobReset.equipped)) {
      for (const item of mobReset.equipped) {
        ...
        if (objectExists) {
          ...
          console.log(
            `⚠️  Object ${item.id} not found for mob equipped in zone ${zoneId}`
          );
        }
      }
    }
  ...
  private mapResetMode(resetMode: string): ResetMode {
    switch (resetMode?.toUpperCase()) {
      ...
      case 'EMPTY':
        return ResetMode.EMPTY;
      case 'NORMAL':
        return ResetMode.NORMAL;
      default:
        return ResetMode.NORMAL;
    }
  }
  ...
  private mapSector(sector: string): Sector {
    switch (sector?.toUpperCase()) {
      ...
      case 'SHALLOWS':
        return Sector.SHALLOWS;
      case 'WATER':
        return Sector.WATER;
      case 'UNDERWATER':
        return Sector.UNDERWATER;
      case 'AIR':
        return Sector.AIR;
      case 'ROAD':
        return Sector.ROAD;
      case 'GRASSLANDS':
        return Sector.GRASSLANDS;
      case 'CAVE':
        return Sector.CAVE;
      case 'RUINS':
        return Sector.RUINS;
      ...
      case 'BEACH':
        return Sector.BEACH;
      case 'UNDERDARK':
        return Sector.UNDERDARK;
      case 'ASTRALPLANE':
        return Sector.ASTRALPLANE;
      case 'AIRPLANE':
        return Sector.AIRPLANE;
      case 'FIREPLANE':
        return Sector.FIREPLANE;
      case 'EARTHPLANE':
        return Sector.EARTHPLANE;
      case 'ETHEREALPLANE':
        return Sector.ETHEREALPLANE;
      case 'AVERNUS':
        return Sector.AVERNUS;
      default:
        return Sector.STRUCTURE;
    }
  }
  ...
  private mapObjectType(type: string): ObjectType {
    switch (type?.toUpperCase()) {
      ...
      case 'DRINKCON':
      case 'DRINKCONTAINER':
        return ObjectType.DRINKCONTAINER;
      case 'KEY':
  ...
  private mapScriptType(attachType: string): ScriptType {
    switch (attachType?.toLowerCase()) {
      case 'mob':
        return ScriptType.MOB;
      case 'object':
        return ScriptType.OBJECT;
      case 'world':
        return ScriptType.WORLD;
      default:
        return ScriptType.WORLD;
    }
  }
    ...
    return flagArray
      .map(flag => {
        ...
        switch (upperFlag) {
          ...
          case 'SCAVENGER':
            return MobFlag.SCAVENGER;
          case 'ISNPC':
            return MobFlag.ISNPC;
          case 'AWARE':
            return MobFlag.AWARE;
          case 'AGGRESSIVE':
            return MobFlag.AGGRESSIVE;
          case 'STAY_ZONE':
            return MobFlag.STAY_ZONE;
          case 'WIMPY':
            return MobFlag.WIMPY;
          case 'AGGRO_EVIL':
          case 'AGGR_EVIL':
            return MobFlag.AGGRO_EVIL;
          case 'AGGRO_GOOD':
          case 'AGGR_GOOD':
            return MobFlag.AGGRO_GOOD;
          case 'AGGRO_NEUTRAL':
          case 'AGGR_NEUTRAL':
            return MobFlag.AGGRO_NEUTRAL;
          case 'MEMORY':
            return MobFlag.MEMORY;
          case 'HELPER':
            return MobFlag.HELPER;
          case 'NO_CHARM':
            return MobFlag.NO_CHARM;
          case 'NO_SUMMOM':
            return MobFlag.NO_SUMMOM;
          case 'NO_SLEEP':
            return MobFlag.NO_SLEEP;
          case 'NO_BASH':
            return MobFlag.NO_BASH;
          case 'NO_BLIND':
            return MobFlag.NO_BLIND;
          case 'MOUNT':
            return MobFlag.MOUNT;
          case 'STAY_SECT':
            return MobFlag.STAY_SECT;
          case 'HATES_SUN':
            return MobFlag.HATES_SUN;
          case 'NO_KILL':
            return MobFlag.NO_KILL;
          case 'TRACK':
            return MobFlag.TRACK;
          case 'ILLUSION':
            return MobFlag.ILLUSION;
          case 'POISON_BITE':
            return MobFlag.POISON_BITE;
          case 'THIEF':
            return MobFlag.THIEF;
          case 'WARRIOR':
            return MobFlag.WARRIOR;
          case 'SORCERER':
            return MobFlag.SORCERER;
          case 'CLERIC':
            return MobFlag.CLERIC;
          case 'PALADIN':
            return MobFlag.PALADIN;
          case 'ANTI_PALADIN':
            return MobFlag.ANTI_PALADIN;
          case 'RANGER':
            return MobFlag.RANGER;
          case 'DRUID':
            return MobFlag.DRUID;
          case 'SHAMAN':
            return MobFlag.SHAMAN;
          case 'ASSASSIN':
            return MobFlag.ASSASSIN;
          case 'MERCENARY':
            return MobFlag.MERCENARY;
          case 'NECROMANCER':
            return MobFlag.NECROMANCER;
          case 'CONJURER':
            return MobFlag.CONJURER;
          case 'MONK':
            return MobFlag.MONK;
          case 'BERSERKER':
            return MobFlag.BERSERKER;
          case 'DIABOLIST':
            return MobFlag.DIABOLIST;
          case 'HASTE':
            return MobFlag.HASTE;
          case 'BLUR':
            return MobFlag.BLUR;
          // Handle some legacy/unknown flags by mapping to closest equivalent or skipping
          case 'TEACHER':
            return MobFlag.SPEC; // Teachers are special mobs
          case 'NOSUMMON':
            return null; // Skip unknown flags
          case 'NOSILENCE':
            return null; // Skip unknown flags
          case 'NOVICIOUS':
            return null; // Skip unknown flags
    ...
    return flagArray
      .map(flag => {
        ...
        switch (normalizedFlag) {
          case 'BLIND':
            return EffectFlag.BLIND;
          case 'INVISIBLE':
            return EffectFlag.INVISIBLE;
          case 'DETECT_ALIGN':
            return EffectFlag.DETECT_ALIGN;
          case 'DETECT_INVIS':
            return EffectFlag.DETECT_INVIS;
          case 'DETECT_MAGIC':
            return EffectFlag.DETECT_MAGIC;
          case 'SENSE_LIFE':
            return EffectFlag.SENSE_LIFE;
          case 'WATERWALK':
            return EffectFlag.WATERWALK;
          case 'SANCTUARY':
            return EffectFlag.SANCTUARY;
          case 'GROUP':
            return EffectFlag.GROUP;
          case 'CURSE':
            return EffectFlag.CURSE;
          case 'INFRAVISION':
            return EffectFlag.INFRAVISION;
          case 'POISON':
            return EffectFlag.POISON;
          case 'PROTECT_EVIL':
            return EffectFlag.PROTECT_EVIL;
          case 'PROTECT_GOOD':
            return EffectFlag.PROTECT_GOOD;
          case 'SLEEP':
            return EffectFlag.SLEEP;
          case 'NO_TRACK':
            return EffectFlag.NO_TRACK;
          case 'SNEAK':
            return EffectFlag.SNEAK;
          case 'HIDE':
            return EffectFlag.HIDE;
          case 'CHARM':
            return EffectFlag.CHARM;
          case 'FLYING':
            return EffectFlag.FLYING;
          case 'WATERBREATH':
            return EffectFlag.WATERBREATH;
          case 'ANGELIC_AURA':
            return EffectFlag.ANGELIC_AURA;
          case 'ETHEREAL':
            return EffectFlag.ETHEREAL;
          case 'MAGICONLY':
            return EffectFlag.MAGICONLY;
          case 'NEXTPARTIAL':
            return EffectFlag.NEXTPARTIAL;
          case 'NEXTNOATTACK':
            return EffectFlag.NEXTNOATTACK;
          case 'SPELL_TURNING':
            return EffectFlag.SPELL_TURNING;
          case 'COMPREHEND_LANG':
            return EffectFlag.COMPREHEND_LANG;
          case 'FIRESHIELD':
            return EffectFlag.FIRESHIELD;
          case 'DEATH_FIELD':
            return EffectFlag.DEATH_FIELD;
          default:
            return null; // Skip unknown flags
        }
      })
  ...
  private mapRoomFlags(flags: any): RoomFlag[] {
    ...
    return flagArray
      .map(flag => {
        ...
        const normalizedFlag = upperFlag.startsWith('ROOM_')
          ? upperFlag.substring(5)
          : upperFlag;
        switch (normalizedFlag) {
          case 'DARK':
            return RoomFlag.DARK;
          case 'DEATH':
            return RoomFlag.DEATH;
          case 'NOMOB':
            return RoomFlag.NOMOB;
          case 'INDOORS':
            return RoomFlag.INDOORS;
          case 'PEACEFUL':
            return RoomFlag.PEACEFUL;
          case 'SOUNDPROOF':
            return RoomFlag.SOUNDPROOF;
          case 'NOTRACK':
            return RoomFlag.NOTRACK;
          case 'NOMAGIC':
            return RoomFlag.NOMAGIC;
          case 'TUNNEL':
            return RoomFlag.TUNNEL;
          case 'PRIVATE':
            return RoomFlag.PRIVATE;
          case 'GODROOM':
            return RoomFlag.GODROOM;
          case 'HOUSE':
            return RoomFlag.HOUSE;
          case 'HOUSECRASH':
            return RoomFlag.HOUSECRASH;
          case 'ATRIUM':
            return RoomFlag.ATRIUM;
          case 'OLC':
            return RoomFlag.OLC;
          case 'BFS_MARK':
            return RoomFlag.BFS_MARK;
          case 'WORLDMAP':
            return RoomFlag.WORLDMAP;
          case 'FERRY_DEST':
            return RoomFlag.FERRY_DEST;
          case 'ISOLATED':
            return RoomFlag.ISOLATED;
          case 'ARENA':
            return RoomFlag.ARENA;
          case 'LARGE':
            return RoomFlag.LARGE;
          case 'MEDIUM_LARGE':
            return RoomFlag.MEDIUM_LARGE;
          case 'MEDIUM':
            return RoomFlag.MEDIUM;
          case 'MEDIUM_SMALL':
            return RoomFlag.MEDIUM_SMALL;
          case 'SMALL':
            return RoomFlag.SMALL;
          case 'VERY_SMALL':
            return RoomFlag.VERY_SMALL;
          case 'ONE_PERSON':
            return RoomFlag.ONE_PERSON;
          case 'EFFECTS_NEXT':
            return RoomFlag.EFFECTS_NEXT;
          default:
            return null; // Skip unknown flags
        }
      })
  ...
  private mapShopFlags(flags: any): ShopFlag[] {
    ...
    return flagArray
      .map(flag => {
        ...
        switch (upperFlag) {
          case 'WILL_FIGHT':
            return ShopFlag.WILL_FIGHT;
          case 'USES_BANK':
            return ShopFlag.USES_BANK;
          default:
            return null; // Skip unknown flags
        }
      })
  ...
  private mapShopTradesWithFlags(flags: any): ShopTradesWith[] {
    ...
    return flagArray
      .map(flag => {
        ...
        switch (upperFlag) {
          case 'ALIGNMENT':
          case 'TRADES_WITH_ALIGNMENT':
            return ShopTradesWith.ALIGNMENT;
          case 'RACE':
          case 'TRADES_WITH_RACE':
            return ShopTradesWith.RACE;
          case 'CLASS':
          case 'TRADES_WITH_CLASS':
            return ShopTradesWith.CLASS;
          default:
            return null; // Skip unknown flags
        }
      })
    ...
    return flagArray
      .map(flag => {
        ...
        switch (upperFlag) {
          ...
          case 'NO_RENT':
            return ObjectFlag.NO_RENT;
          case 'INVISIBLE':
            return ObjectFlag.INVISIBLE;
          case 'MAGIC':
            return ObjectFlag.MAGIC;
          case 'NO_DROP':
            return ObjectFlag.NO_DROP;
          case 'PERMANENT':
            return ObjectFlag.PERMANENT;
          case 'ANTI_GOOD':
            return ObjectFlag.ANTI_GOOD;
          case 'ANTI_EVIL':
            return ObjectFlag.ANTI_EVIL;
          case 'ANTI_NEUTRAL':
            return ObjectFlag.ANTI_NEUTRAL;
          case 'NO_SELL':
            return ObjectFlag.NO_SELL;
          case 'FLOAT':
            return ObjectFlag.FLOAT;
          case 'NO_FALL':
            return ObjectFlag.NO_FALL;
          case 'ELVEN':
            return ObjectFlag.ELVEN;
          case 'DWARVEN':
            return ObjectFlag.DWARVEN;
          default:
            return null; // Skip unknown flags
        }
      })
  ...
  private mapWearFlags(flags: any): WearFlag[] {
    ...
    return flagArray
      .map(flag => {
        ...
        switch (upperFlag) {
          case 'TAKE':
            return WearFlag.TAKE;
          case 'FINGER':
            return WearFlag.FINGER;
          case 'NECK':
            return WearFlag.NECK;
          case 'BODY':
            return WearFlag.BODY;
          case 'HEAD':
            return WearFlag.HEAD;
          case 'LEGS':
            return WearFlag.LEGS;
          case 'FEET':
            return WearFlag.FEET;
          case 'HANDS':
            return WearFlag.HANDS;
          case 'ARMS':
            return WearFlag.ARMS;
          case 'SHIELD':
            return WearFlag.SHIELD;
          case 'ABOUT':
            return WearFlag.ABOUT;
          case 'WAIST':
            return WearFlag.WAIST;
          case 'WRIST':
            return WearFlag.WRIST;
          case 'WIELD':
            return WearFlag.WIELD;
          case 'HOLD':
            return WearFlag.HOLD;
          case 'TWO_HAND_WIELD':
          case 'TWO-HAND-WIELD':
            return WearFlag.TWO_HAND_WIELD;
          default:
            return null; // Skip unknown flags
        }
      })
  ...
  private mapLifeForce(lifeForce: any): LifeForce {
    ...
    switch (lifeForceStr) {
      case 'LIFE':
        return LifeForce.LIFE;
      case 'UNDEAD':
        return LifeForce.UNDEAD;
      case 'MAGIC':
        return LifeForce.MAGIC;
      case 'CELESTIAL':
        return LifeForce.CELESTIAL;
      case 'DEMONIC':
        return LifeForce.DEMONIC;
      case 'ELEMENTAL':
        return LifeForce.ELEMENTAL;
      default:
        return LifeForce.LIFE;
    }
  }
  ...
  private mapComposition(composition: any): Composition {
    ...
    switch (compStr) {
      case 'FLESH':
        return Composition.FLESH;
      case 'EARTH':
        return Composition.EARTH;
      case 'AIR':
      ...
      case 'ICE':
        return Composition.ICE;
      case 'MIST':
        return Composition.MIST;
      case 'ETHER':
        return Composition.ETHER;
      case 'METAL':
        return Composition.METAL;
      case 'STONE':
        return Composition.STONE;
      case 'BONE':
        return Composition.BONE;
      case 'LAVA':
        return Composition.LAVA;
      case 'PLANT':
        return Composition.PLANT;
      default:
        return Composition.FLESH;
    }
  }
  ...
  private mapStance(stance: any): Stance {
    ...
    switch (stanceStr) {
      ...
      case 'MORT':
        return Stance.MORT;
      case 'INCAPACITATED':
        return Stance.INCAPACITATED;
      case 'STUNNED':
        return Stance.STUNNED;
      case 'SLEEPING':
        return Stance.SLEEPING;
      case 'RESTING':
        return Stance.RESTING;
      case 'ALERT':
        return Stance.ALERT;
      case 'FIGHTING':
        return Stance.FIGHTING;
      default:
        return Stance.ALERT;
    }
  }
  ...
  private mapDamageType(damageType: any): DamageType {
    ...
    switch (damageStr) {
      ...
      case 'STING':
        return DamageType.STING;
      case 'WHIP':
        return DamageType.WHIP;
      case 'SLASH':
        return DamageType.SLASH;
      case 'BITE':
        return DamageType.BITE;
      case 'BLUDGEON':
      ...
      case 'CLAW':
        return DamageType.CLAW;
      case 'MAUL':
        return DamageType.MAUL;
      case 'THRASH':
        return DamageType.THRASH;
      case 'PIERCE':
        return DamageType.PIERCE;
      case 'BLAST':
        return DamageType.BLAST;
      case 'PUNCH':
        return DamageType.PUNCH;
      case 'STAB':
